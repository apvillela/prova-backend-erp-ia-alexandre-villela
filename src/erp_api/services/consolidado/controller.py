...
from erp_api.services.consolidado import fontes
from erp_api.services.consolidado.schemas import ConsolidadoResponse, FonteResultado
...
async def _consultar_fonte(
    ...
    tentativas = settings.consolidado_tentativas
    for tentativa in range(1, tentativas + 1):
        try:
            dados = await asyncio.wait_for(consulta(), timeout=settings.consolidado_timeout)
        except TimeoutError:
            log.warning(f"Timeout na fonte {nome} (tentativa {tentativa}/{tentativas})")
            erro = f"Timeout após {settings.consolidado_timeout}s"
